...
void APIENTRY opengl_debug_callback(GLenum source, GLenum type, unsigned int id, GLenum severity, GLsizei length,
	...
	switch (severity) {
		case GL_DEBUG_SEVERITY_HIGH: {
			TL_CORE_ERROR("OpenGL Debug message({0}) : {1}\n"
			              "   Source   : {2}\n"
			              "   Type     : {3}\n"
			              "   Severity : HIGH",
		...
		case GL_DEBUG_SEVERITY_MEDIUM: {
			TL_CORE_WARN("OpenGL Debug message({0}) : {1}\n"
			             "   Source   : {2}\n"
			             "   Type     : {3}\n"
			             "   Severity : MEDIUM",
			    id, message, source_name, type_name);
		...
		case GL_DEBUG_SEVERITY_LOW: {
			TL_CORE_TRACE("OpenGL Debug message({0}) : {1}\n"
			              "   Source   : {2}\n"
			              "   Type     : {3}\n"
			              "   Severity : LOW",
			    id, message, source_name, type_name);
		...
		case GL_DEBUG_SEVERITY_NOTIFICATION: {
			TL_CORE_DEBUG("OpenGL Debug message({0}) : {1}\n"
			              "   Source   : {2}\n"
			              "   Type     : {3}\n"
